...
| Low ops, and a third-party API is acceptable | An inexpensive hosted open model on a marketplace, provider and quantization pinned (a small Mistral or DeepSeek open model), the router on it and the verdict on it or a capable fallback | about $0.09 to $0.30 per 1,000 triage nodes, zero schema errors on the golden set | The alert content leaves your perimeter to a third-party API; these are single measured runs with prices as snapshots |
