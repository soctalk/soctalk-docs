...
This guide covers what an MSSP actually needs from a multi-tenant Wazuh deployment, the three patterns teams try in practice, and what production-grade isolation requires beyond the SIEM itself. It is the architecture SocTalk implements as open source (Apache 2.0); the reference pages linked throughout go a level deeper on each layer.

## What an MSSP needs that Wazuh doesn't provide

Three requirements come up in every MSSP deployment conversation:

1. **Isolation you can defend in a customer security review.** "Customer A cannot read customer B's alerts" has to hold at the data layer, the network layer, and the agent-enrollment layer — not just in the dashboard.
...
The problem is that the separation is a read-side filter, not a boundary. The manager itself is shared: one ruleset, one `authd` secret, one API, one upgrade window for everyone. A misconfigured role exposes every customer at once, and per-customer rule packs or retention policies are impossible without affecting the rest.
...
Of the three, pattern 3 is the one built to deliver both real isolation and onboarding speed — but only if the control plane exists. Namespaces alone are a naming convention, not a security boundary. The rest of this guide is about what makes the boundary real.

## Production isolation is more than the SIEM

A per-tenant Wazuh stack isolates the SIEM data. An MSSP platform also has cross-tenant state — cases, review queues, audit logs, integration configs — and that layer needs its own enforcement.

### Data layer: Postgres row-level security, forced and tested

Application-level `WHERE tenant_id = ?` filtering is one forgotten clause away from a cross-tenant leak. The database should enforce tenancy itself. The pattern:

- Every tenant-scoped table carries RLS policies keyed on a per-transaction `app.current_tenant_id` setting. An unset context yields **zero rows** — defensive-zero, not leakage.
...
SocTalk uses Cilium as the supported CNI (NetworkPolicy enforcement, FQDN-based egress for hostname-addressed LLM endpoints, Hubble flow observability for debugging isolation questions). Be aware of the V1 hedge: the fully FQDN-pinned per-tenant egress allowlist is the design destination, and the current chart renders simpler policies — permissive control-plane egress and broad TCP/443 egress for the per-tenant worker. The rendered templates are in the repo; read [NetworkPolicy design](/reference/network-policy) for both the shipped policies and the target architecture.

### Agent enrollment: per-tenant endpoints and secrets

The subtlest failure mode: customer A's agent registering with customer B's manager. Wazuh's agent protocol on 1514/TCP is a proprietary encrypted stream, not standard TLS — there is no SNI to route on, so hostname-inspecting L4 proxies silently break. Routing has to be by destination address: each tenant gets its own DNS name (`acme.soc.mssp.example.com`) resolving to a per-tenant L4 endpoint, with a per-tenant-port fallback when IPs are scarce.
...
- **The bottleneck is usually the per-tenant Wazuh indexer** — each is a Java process with its own heap. Plan ~6–8 GB RAM and ~1.5 vCPU per production tenant.
...
SocTalk is an open-source (Apache 2.0, no community/enterprise split) implementation of pattern 3: one control plane, one `soctalk-tenant` Helm release per customer, on your own Kubernetes 1.30+ — K3s, EKS, AKS, or GKE.
...
The full lifecycle — states, phases, quotas, recovery paths — is in [Tenant lifecycle](/tenant-lifecycle). To run it: the [install guide](/install) covers a production cluster in about an hour, and the [demo VM](/quickstart-vm) boots a working multi-tenant install with a demo tenant in about five minutes.
