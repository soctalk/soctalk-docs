...
# Building an open-source SOC stack with Wazuh, TheHive, Cortex, and MISP: assembled vs integrated

There is a canonical free-and-open-source SOC stack, and it has been roughly the same four names for years: Wazuh for detection, TheHive for case management, Cortex for observable analysis, MISP for threat intelligence. All four are mature projects with years of production use behind them, and together they cover most of what a commercial SOC suite sells. The catch is the word *together*. The integration between the tools is a project you build and then own.

This guide covers what each piece does, what assembling them actually costs, how the requirements change when you run more than one organization's security, and where SocTalk fits, which is *on top of* this stack rather than in place of it.
...
**Cortex** is TheHive's companion for observable analysis. You hand it an IP, hash, domain, or URL, and its analyzer plugins fan out to reputation and sandbox services, from VirusTotal and AbuseIPDB to Hybrid Analysis and dozens more, then bring back a verdict. It turns "here's a hash" into "here's what the world knows about this hash."
...
That is four tools covering four distinct jobs, all open source, and on paper a complete SOC.
...
The tools themselves are blameless here; this is simply what composing independent projects entails. The integration layer amounts to a fifth product, except nobody ships it, documents it, or upgrades it for you.
...
- **Per-customer stacks multiply the tax.** Ten customers on dedicated stacks means ten Wazuh managers and indexers to deploy, upgrade, and back up, plus ten copies of your glue.
- **Onboarding must be repeatable.** Customer eleven should take a command rather than a week of wiki archaeology. Hand-built stacks drift, and drift eventually surfaces as an incident.
...
## Where SocTalk fits: a control plane on top of the stack

[SocTalk](https://github.com/soctalk/soctalk) leaves all four tools in place. It is an Apache 2.0 multi-tenant control plane and AI triage layer built *around* this stack, for MSPs and MSSPs running it on their own Kubernetes:
...
**Know the V1 integration surface before you plan around it:**
...
SocTalk packages the multi-tenant Wazuh plane and the triage layer, and *connects to* the TheHive/Cortex/MISP instances you operate. The bundled-subchart convenience stays on the roadmap; this release does not include it.

## When to build the stack yourself, and when to deploy SocTalk

Both paths are open source, so the choice rests on operational criteria:
