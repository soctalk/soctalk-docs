...
description: How to run AI triage on Wazuh alerts in production - a deterministic funnel before any LLM call, code-level guardrails, human review, and honest cost numbers.
---
...
That prototype works. It also fails in production, in predictable ways. This guide covers why, and the architecture that holds up when AI triage of Wazuh alerts has to run unattended against a real alert volume — the architecture SocTalk implements.
...
**The model has no context and no floor.** An LLM reading one alert in isolation has no memory of what an analyst decided yesterday, no picture of the organization's own state — so it cannot tell a sanctioned change from an attack that produces a byte-identical alert — and no guarantee it won't confidently close over a real indicator of compromise. A hallucinated "benign" verdict on a real intrusion is not a quality problem you can tolerate at any rate; it's a suppressed detection.
...
To be fair: the webhook prototype is a fine way to convince yourself LLMs can reason about alerts. It's the *architecture around* the model that's missing.
...
- **Coalescing** groups repeated alerts from the same rule on the same asset within a five-minute window into a single case — a burst of one detection becomes one case, not thousands.
...
## Human-in-the-loop is a hard property, not a setting

Every `escalate` verdict goes through human review. There is no bypass: an AI-only "auto-approve" mode is not implemented in SocTalk (removing the gate is a roadmap item, planned as an admin-gated, audited toggle — not a quiet default). In V1 the review surface is the dashboard queue, showing the AI's full rationale and the observable evidence with its enrichment. Analyst decisions — approve, reject, more info — write append-only audit rows with identity, timestamp, and rationale, never editable after submit. A proposed close touching a sensitive asset (a PCI-classified host, say) is held for human sign-off even when the model is confident.

The same stance governs response: a containment action such as isolating an endpoint or disabling an account is *always* raised as a proposal an analyst approves first. The model never takes a containment action on its own, and dispatch happens server-side, never from the model's loop. SocTalk is a copilot, not an analyst replacement — the value is compression: the same analyst team can handle 5–10× the alert volume because routine cases auto-close and only the unclear ones reach human review.
...
- **What does it cost?** Highly variable, but as an order of magnitude: roughly **$9/day per tenant** at ~30 alerts/day on a budget OpenAI-compatible setup, dropping 5–10× with a cheaper fast model. Treat that as a starting estimate, not a quote.
...
- [How it works](/how-it-works) — the full pipeline story: the deterministic funnel, the two model roles, the three-site safety floor.
- [AI pipeline](/ai-pipeline) — the LangGraph state machine: supervisor, workers, verdict, run lifecycle.
- [Triage policies](/triage-policies) — authoring deterministic guardrails in the no-code editor, shadow-then-activate.
- [Human review](/human-review) — the review queue and the analyst decision contract.
