...
This produces a writable, dynamically-sized `soctalk-demo-0.2.1.vdi` (a few GB on disk). `VBoxManage` ships with VirtualBox, on Windows it's in `C:\Program Files\Oracle\VirtualBox\`.
...
**Virtual Hard disk**: choose **Use an Existing Virtual Hard Disk File** and select the `soctalk-demo-0.2.1.vdi` you converted:
