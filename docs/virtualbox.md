...
```bash
VBoxManage clonemedium disk soctalk-demo-0.1.2.vmdk soctalk-demo-0.1.2.vdi --format VDI
```

This produces a writable, dynamically-sized `soctalk-demo-0.1.2.vdi` (a few GB on disk). `VBoxManage` ships with VirtualBox — on Windows it's in `C:\Program Files\Oracle\VirtualBox\`.
...
**Virtual Hard disk** — choose **Use an Existing Virtual Hard Disk File** and select the `soctalk-demo-0.1.2.vdi` you converted:
...
VBoxManage closemedium disk soctalk-demo-0.1.2.vdi --delete
```
