...
这将生成一个可写、动态分配大小的 `soctalk-demo-0.2.1.vdi`（磁盘上占用数 GB）。`VBoxManage` 随 VirtualBox 一起提供——在 Windows 上位于 `C:\Program Files\Oracle\VirtualBox\`。
...
**Virtual Hard disk**——选择 **Use an Existing Virtual Hard Disk File** 并选中你转换得到的 `soctalk-demo-0.2.1.vdi`：
