...
- **Portas de SIEM não padrão (somente v0.2.0).** A NetworkPolicy de egress do tenant deriva o *host* do SIEM externo das URLs que você fornece, mas fixa as *portas* em 9200 e 55000. Um Wazuh acessível em outra porta é descartado na camada de rede mesmo que o tenant chegue a `active`, o Secret de credenciais seja escrito e o adapter siga enviando heartbeats: o único sintoma é `ingest_failed: All connection attempts failed` no log do adapter. Verificado no mesmo cluster variando apenas as portas: um indexer publicado em NodePort `:31437` nunca conectou, enquanto o mesmo Wazuh em `:9200` conectou e autenticou. Até a correção ([soctalk#147](https://github.com/soctalk/soctalk/issues/147)) ser lançada, exponha o indexer e a Manager API ao SocTalk nas portas 9200 e 55000. Depois disso, as portas são lidas das URLs fornecidas e qualquer porta funciona. **Corrigido na v0.2.1** ([soctalk#147](https://github.com/soctalk/soctalk/issues/147)): as portas são lidas das URLs fornecidas e qualquer porta funciona.
