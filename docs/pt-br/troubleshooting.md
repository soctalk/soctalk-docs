...
| ImagePullBackOff no `soctalk-adapter` / `soctalk-runs-worker` do tenant | `kubectl -n tenant-<slug> describe pod` mostra falha de pull para uma tag de imagem do tenant | O chart as fixa na release: `tenantProvisioning.adapterImageTag` e `runsWorkerImageTag` usam por padrão a versão do próprio chart (`0.2.1`). Em caso de falha, veja qual tag a API entrega ao provisioner — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — e defina uma **versão de release publicada**. Não use `latest`: uma tag móvel faz o tenant rodar código não publicado e a build muda sem aviso. |
