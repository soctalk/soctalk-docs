...
Questo produce un `soctalk-demo-0.2.1.vdi` scrivibile e dimensionato dinamicamente (pochi GB su disco). `VBoxManage` è incluso con VirtualBox, su Windows si trova in `C:\Program Files\Oracle\VirtualBox\`.
...
**Virtual Hard disk**: scegli **Use an Existing Virtual Hard Disk File** e seleziona il `soctalk-demo-0.2.1.vdi` che hai convertito:
