...
### Override di render del tenant

`soctalk-system` porta anche le chiavi che il controller usa quando renderizza e installa ciascun chart per-tenant. Fissano il riferimento al chart del tenant e i tag delle immagini firmate da SocTalk che il control plane inietta in ogni render del tenant, così un'installazione aggiorna l'intera flotta di tenant modificando questi valori in un unico punto.
...
### Subchart OSS upstream (Wazuh / linux-ep)

Mantenuti come chart sibling sotto `charts/` (`charts/wazuh`, `charts/linux-ep`) e riferiti per percorso relativo, non scaricati al momento dell'installazione. `Chart.yaml` li elenca come dipendenze locali, ciascuna gestita dalla propria condizione di componente:
...
TheHive e Cortex **non** sono subchart vendored. Sono integrazioni esterne configurate per tenant (vedi /it-it/integrate/thehive e /it-it/integrate/cortex).
