...
Il pattern ingenuo (webhook Wazuh → prompt LLM → verdict) ha tre problemi strutturali, e nessuno di questi si risolve con prompt migliori.
...
**Il modello non ha contesto e non ha un limite inferiore.** Un LLM che legge un alert in isolamento non ricorda cosa ha deciso un analista ieri e non ha una visione dello stato dell'organizzazione, quindi non può distinguere una modifica autorizzata da un attacco che produce un alert identico byte per byte. Nulla garantisce che non chiuda con sicurezza sopra un vero indicatore di compromissione, e un verdict "benigno" allucinato su un'intrusione reale è una detection soppressa; nessun tasso di errori di quel tipo è tollerabile.

**Non c'è audit trail né cancello di controllo.** Un workflow che pubblica il verdict del modello direttamente in un canale non conserva traccia delle evidenze su cui il verdict si è basato, non registra l'identità del revisore e non ha alcun meccanismo per impedire che un verdict sbagliato diventi un caso chiuso.
...
## Guardrail come dati, verdict vincolati dal codice

La seconda correzione tratta il verdict del modello come una proposta che solo un cancello deterministico può trasformare in una decisione definitiva. La regola di SocTalk è *"l'LLM propone; un cancello deterministico dispone"*.

Le [policy di triage](/it-it/triage-policies) sono dati, regole dichiarative eseguite da un unico interprete, che agiscono su quattro cancelli: un resolver, un cancello pre-decisione (un verdict non è valido finché non sono stati eseguiti i passi di evidenza richiesti), un guard post-verdict e un **safety floor**. Il floor è a livello di codice e non aggirabile, applicato in tre punti indipendenti (worker, server, ingest). Nessuna chiusura automatica può scattare sopra un IOC noto, un record di autorizzazione contraddetto, un indicatore non verificato, un incidente correlato attivo, un kill switch, o oltre il limite di volume (default 500 chiusure automatiche ogni 24 ore). I kill switch (`SOCTALK_AUTO_CLOSE_KILL` per l'intera installazione, oppure per singolo tenant) trasformano istantaneamente ogni chiusura automatica in una promozione. È il controllo a cui ricorri nel mezzo di un incidente.
...
- **Divisione fast/reasoning.** Routing e worker usano un modello veloce; solo il verdict usa un modello di reasoning. I default sono `claude-sonnet-4-20250514` per entrambi, sovrascrivibili per tenant (`SOCTALK_FAST_MODEL` / `SOCTALK_REASONING_MODEL`).
...
- [Pipeline AI](/it-it/ai-pipeline) copre la macchina a stati LangGraph: supervisore, worker, verdict, ciclo di vita delle esecuzioni.
