...
| ImagePullBackOff su `soctalk-adapter` / `soctalk-runs-worker` del tenant | `kubectl -n tenant-<slug> describe pod` mostra un errore di pull per un tag immagine del tenant | Il chart li fissa alla release: `tenantProvisioning.adapterImageTag` e `runsWorkerImageTag` usano per default la versione del chart (`0.2.1`). In caso di errore, controlla quale tag l'API passa al provisioner — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — e impostali su una **versione di release pubblicata**. Non impostarli su `latest`: un tag mobile fa girare al tenant codice non rilasciato e la build cambia senza preavviso. |
