...
L'azione sul verdetto avviene di nuovo sul server, in modo deterministico, dopo il completamento della run. Questo tiene il modello fuori dal loop che tocca i sistemi esterni.
...
- **Una run consulta il modello in soli due ruoli**, il routing e il verdetto finale, e molti casi si chiudono in modo deterministico senza alcuna chiamata al modello. L'enrichment è orchestrazione deterministica di tool, non classificazione del modello per singolo alert.
...
- [Response Playbooks](/it-it/response-playbooks), trasformare un verdetto in azione governata.
