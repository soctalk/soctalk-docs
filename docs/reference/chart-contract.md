...
`soctalk-system` also carries the keys the controller uses when it renders and installs each per-tenant chart. They pin the tenant chart reference and the SocTalk-authored image tags the control plane injects into every tenant render, so an install upgrades its whole tenant fleet by bumping these in one place.
...
Kept as sibling charts under `charts/` (`charts/wazuh`, `charts/linux-ep`) and referenced by relative path, not fetched at install time. `Chart.yaml` lists them as local dependencies, each gated by its component condition:
...
TheHive and Cortex are **not** vendored subcharts. They are external integrations configured per tenant (see /integrate/thehive and /integrate/cortex).
