...
Esto produce un `soctalk-demo-0.2.1.vdi` escribible y de tamaño dinámico (unos pocos GB en disco). `VBoxManage` se incluye con VirtualBox; en Windows está en `C:\Program Files\Oracle\VirtualBox\`.
...
**Virtual Hard disk**: elige **Use an Existing Virtual Hard Disk File** y selecciona el `soctalk-demo-0.2.1.vdi` que convertiste:
