...
| La instalación en distribuciones de la familia RHEL expira en Helm mientras Postgres está `1/1 Running` | `sudo /usr/local/bin/k3s kubectl -n soctalk-system logs -l app.kubernetes.io/component=api -c db-init` muestra `No route to host` | firewalld está descartando el tráfico de los pods en el bridge de flannel. Aplica las reglas de las [notas de paquetes RHEL](/es-419/os-packages#firewalld); una instalación que aún está esperando se recupera sola |
...
| ImagePullBackOff en el `soctalk-adapter` / `soctalk-runs-worker` del tenant | `kubectl -n tenant-<slug> describe pod` muestra una falla de pull para un tag de imagen del tenant | El chart los fija al release: `tenantProvisioning.adapterImageTag` y `runsWorkerImageTag` usan por defecto la versión del propio chart (`0.2.1`). Ante una falla de pull, revisa qué tag entrega la API al provisioner — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — y defínelos con una **versión de release publicada**. No los pongas en `latest`: un tag móvil hace que el tenant ejecute código no publicado y que la build cambie sin aviso. |
