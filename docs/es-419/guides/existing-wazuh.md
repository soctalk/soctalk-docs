...
Una advertencia si republicas el indexer en otra dirección o puerto para que SocTalk lo alcance, por ejemplo mediante NodePort, port-forward o un proxy inverso. Prueba las credenciales **a través de la URL exacta que vas a configurar**, no contra el `:9200` propio del indexer. En un banco de pruebas armado así vimos el mismo indexer, los mismos pods y las mismas credenciales responder `200` en `:9200` y `401` a través del puerto republicado, reproducible con un `curl` simple desde el host y por lo tanto ajeno a SocTalk. No investigamos la causa; la lección práctica es que la ruta republicada es algo aparte y merece su propia comprobación:
...
Si devuelve 401 mientras el puerto propio del indexer devuelve 200, corrige la exposición antes del onboarding: SocTalk reproducirá fielmente ese 401.
...
- **Puertos de SIEM no estándar (solo v0.2.0).** La NetworkPolicy de egress del tenant deriva el *host* del SIEM externo de las URL que proporcionas, pero fija los *puertos* en 9200 y 55000. Un Wazuh accesible en otro puerto se descarta en la capa de red aunque el tenant llegue a `active`, el Secret de credenciales se escriba y el adapter siga enviando heartbeats: el único síntoma es `ingest_failed: All connection attempts failed` en el log del adapter. Verificado en el mismo clúster variando solo los puertos: un indexer publicado en NodePort `:31437` nunca conectó, mientras que el mismo Wazuh en `:9200` conectó y autenticó. Hasta que llegue la corrección ([soctalk#147](https://github.com/soctalk/soctalk/issues/147)), expón el indexer y la Manager API a SocTalk en los puertos 9200 y 55000. Después, los puertos se leen de las URL que proporcionas y cualquier puerto funciona.
