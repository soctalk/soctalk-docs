...
![The evolution of SOC triage: rules, machine learning, language models, and the agentic generation SocTalk represents](/diagrams/soc-evolution.svg)
...
SocTalk is the agentic generation. Where the first wave pointed a model at one alert, SocTalk runs an agentic loop around the model: the model directs a deterministic investigation, reasons over the whole correlated case, and returns a verdict that drives governed action, with a human gating anything dangerous. All of it runs inside deterministic guardrails. It keeps the guarantees of the rules era in code, and deliberately skips the opaque middle. The noise collapse that machine learning set out to do is done deterministically instead, by coalescing, correlation, and rule-based close, so nothing in the decision path is a trained black box. The model is spent only on the ambiguous cases. Then two things none of the earlier generations had are added on top: the pipeline remembers what analysts decide, and a human gates anything that reaches into a live system.
