...
Das naive Muster (Wazuh-Webhook → LLM-Prompt → Verdict) hat drei strukturelle Probleme, von denen keines durch besseres Prompting behoben wird.
...
**Das Modell hat keinen Kontext und keine Untergrenze.** Ein LLM, das eine Warnung isoliert liest, hat keine Erinnerung daran, was ein Analyst gestern entschieden hat, und kein Bild vom eigenen Zustand der Organisation. Es kann daher eine genehmigte Änderung nicht von einem Angriff unterscheiden, der eine byte-identische Warnung erzeugt. Nichts garantiert, dass es nicht selbstbewusst über einen echten Kompromittierungsindikator hinweg schließt, und ein halluziniertes „gutartig“-Verdict zu einem echten Einbruch ist eine unterdrückte Erkennung; keine Rate davon ist tolerierbar.

**Es gibt keinen Audit-Trail und kein Gate.** Ein Workflow, der das Verdict des Modells direkt in einen Kanal postet, hat keine Aufzeichnung darüber, auf welchen Belegen das Verdict beruhte, keine Prüferidentität und keinen Mechanismus, der ein schlechtes Verdict davon abhält, zu einem geschlossenen Fall zu werden.
...
- **Aufteilung in schnelles und Reasoning-Modell.** Routing und Worker nutzen ein schnelles Modell; nur das Verdict nutzt ein Reasoning-Modell. Standard ist `claude-sonnet-4-20250514` für beide, überschreibbar pro Mandant (`SOCTALK_FAST_MODEL` / `SOCTALK_REASONING_MODEL`).
...
- [AI-Pipeline](/de-de/ai-pipeline) behandelt die LangGraph-State-Machine: Supervisor, Worker, Verdict, Lauf-Lebenszyklus.
