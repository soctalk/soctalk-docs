...
| Installation auf RHEL-Derivaten läuft bei Helm in einen Timeout, während Postgres `1/1 Running` ist | `sudo /usr/local/bin/k3s kubectl -n soctalk-system logs -l app.kubernetes.io/component=api -c db-init` zeigt `No route to host` | firewalld verwirft den Pod-Verkehr auf der flannel-Bridge. Wende die Regeln aus den [RHEL-Paketnotizen](/de-de/os-packages#firewalld) an; eine noch wartende Installation erholt sich von selbst |
...
| ImagePullBackOff beim Mandanten-`soctalk-adapter` / `soctalk-runs-worker` | `kubectl -n tenant-<slug> describe pod` zeigt einen Pull-Fehler für ein Mandanten-Image-Tag | Das Chart pinnt diese auf das Release: `tenantProvisioning.adapterImageTag` und `runsWorkerImageTag` verwenden standardmäßig die Chart-Version (`0.2.1`). Prüfen Sie bei einem Pull-Fehler, welches Tag die API dem Provisioner übergibt — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — und setzen Sie die Werte auf eine **veröffentlichte Release-Version**. Setzen Sie sie **nicht** auf `latest`: Ein bewegliches Tag bedeutet, dass der Mandant unveröffentlichten Code ausführt und sich der Build unbemerkt ändert. |
