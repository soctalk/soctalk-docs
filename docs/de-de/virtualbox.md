...
Dies erzeugt ein beschreibbares, dynamisch dimensioniertes `soctalk-demo-0.2.1.vdi` (einige GB auf der Festplatte). `VBoxManage` wird mit VirtualBox ausgeliefert, unter Windows liegt es in `C:\Program Files\Oracle\VirtualBox\`.
...
**Virtual Hard disk**: wähle **Use an Existing Virtual Hard Disk File** und selektiere die von dir konvertierte `soctalk-demo-0.2.1.vdi`:
