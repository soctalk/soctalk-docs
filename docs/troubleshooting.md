...
| ImagePullBackOff on tenant `soctalk-adapter` / `soctalk-runs-worker` | `kubectl -n tenant-<slug> describe pod` shows a pull failure for a tenant image tag | The chart pins these to the release: `tenantProvisioning.adapterImageTag` and `runsWorkerImageTag` default to the chart's own version (`0.2.1`). If you see a pull failure, check what tag the API is actually handing the provisioner — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — and set `tenantProvisioning.adapterImageTag` / `runsWorkerImageTag` to a **published release version**. Do not set them to `latest`: a moving tag means the tenant silently runs unreleased code, and which build it runs changes under you. |
