...
| L'installation sur une distribution de la famille RHEL expire sur Helm alors que Postgres est `1/1 Running` | `sudo /usr/local/bin/k3s kubectl -n soctalk-system logs -l app.kubernetes.io/component=api -c db-init` affiche `No route to host` | firewalld rejette le trafic des pods sur le bridge flannel. Appliquez les règles des [notes de paquets RHEL](/fr-fr/os-packages#firewalld) ; une installation encore en attente se rétablit d'elle-même |
...
| ImagePullBackOff sur le `soctalk-adapter` / `soctalk-runs-worker` d'un tenant | `kubectl -n tenant-<slug> describe pod` montre un échec de pull pour un tag d'image de tenant | Le chart les épingle sur la release : `tenantProvisioning.adapterImageTag` et `runsWorkerImageTag` utilisent par défaut la version du chart (`0.2.1`). En cas d'échec, vérifiez le tag que l'API transmet au provisioner — `kubectl -n soctalk-system get deploy soctalk-system-api -o yaml \| grep SOCTALK_TENANT_.*_IMAGE_TAG` — puis définissez une **version de release publiée**. Ne les mettez pas à `latest` : un tag mouvant fait tourner du code non publié et la build change à votre insu. |
