...
```bash
VBoxManage clonemedium disk soctalk-demo-0.2.1.vmdk soctalk-demo-0.2.1.vdi --format VDI
```

Cela produit un fichier `soctalk-demo-0.2.1.vdi` inscriptible et à taille dynamique (quelques Go sur le disque). `VBoxManage` est fourni avec VirtualBox, sous Windows, il se trouve dans `C:\Program Files\Oracle\VirtualBox\`.
...
**Disque dur virtuel**: choisissez **Utiliser un fichier de disque dur virtuel existant** et sélectionnez le fichier `soctalk-demo-0.2.1.vdi` que vous avez converti :
...
VBoxManage closemedium disk soctalk-demo-0.2.1.vdi --delete
```
