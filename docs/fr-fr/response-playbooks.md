...
`external_action` est celle qui nécessite une approbation. Elle envoie une action nommée conjointement avec l'enveloppe signée vers un endpoint configuré par l'opérateur, et c'est là que se trouve le vrai travail, isoler un endpoint ou désactiver un compte, en dehors de SocTalk derrière un contrat stable. Elle ne s'exécute jamais sans qu'un analyste ne l'approuve au préalable.

Un détail garde `external_action` en sécurité. Un auteur de playbook nomme un endpoint et une action, jamais une URL. L'opérateur associe ce nom d'endpoint à une URL réelle et à un secret de signature dans la politique de tenant `response_action_endpoints`, de sorte qu'un auteur peut demander d'isoler sur l'endpoint `edr` mais ne peut pas choisir où la requête part réellement. Chaque requête est signée par HMAC, et elle refuse d'atteindre une adresse privée ou de type link-local.
...
Les administrateurs créent des playbooks de réponse depuis la page **Response Playbooks** pendant qu'un tenant est épinglé, sans aucun YAML requis. Cette section détaille la construction du playbook `isolate-lateral-movement-endpoint` à partir du schéma ci-dessus, de bout en bout. Il propose d'isoler un endpoint lors d'une escalade de mouvement latéral à haute sévérité, notifie le SOC et annote le cas.
...
Maintenant l'action d'isolement. Sur escalade, ajoutez `external_action`, celle marquée « needs approval ». Nommez l'endpoint que l'opérateur a configuré et l'action, qui est `isolate_endpoint`, dans ses params, et vous n'entrez jamais d'URL. Ajoutez une condition pour qu'elle ne se déclenche que sur une escalade à haute sévérité.
